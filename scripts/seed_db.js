...
let dbPath;
if (fs.existsSync(path.resolve(__dirname, '../backend/src/config/database.js'))) {
  // Local development
  dbPath = '../backend/src/config/database';
} else if (fs.existsSync(path.resolve(__dirname, '../src/config/database.js'))) {
  // Docker container
  dbPath = '../src/config/database';
} else {
  // Fallback for backend/scripts location
  dbPath = './src/config/database';
}
