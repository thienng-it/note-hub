...
describe('CRUD Timestamp Tests', () => {
  ...
  describe('User CRUD Operations', () => {
    ...
    it('should update updated_at when updating a user', async () => {
      const userBefore = await db.queryOne(`SELECT * FROM users WHERE id = ?`, [userId]);

      // Wait a bit to ensure timestamp difference
      await new Promise((resolve) => setTimeout(resolve, 100));

      await db.run(`UPDATE users SET email = ? WHERE id = ?`, ['newemail@example.com', userId]);

      const userAfter = await db.queryOne(`SELECT * FROM users WHERE id = ?`, [userId]);
  ...
  describe('Note CRUD Operations', () => {
    ...
    it('should set created_at and updated_at when creating a note', async () => {
      const result = await db.run(`INSERT INTO notes (title, body, owner_id) VALUES (?, ?, ?)`, [
        'Test Note',
        'Note content',
        userId,
      ]);
    ...
    it('should update updated_at when updating a note', async () => {
      const noteBefore = await db.queryOne(`SELECT * FROM notes WHERE id = ?`, [noteId]);

      // Wait a bit to ensure timestamp difference
      await new Promise((resolve) => setTimeout(resolve, 100));

      await db.run(`UPDATE notes SET title = ? WHERE id = ?`, ['Updated Note Title', noteId]);

      const noteAfter = await db.queryOne(`SELECT * FROM notes WHERE id = ?`, [noteId]);
  ...
  describe('Task CRUD Operations', () => {
    ...
    it('should update updated_at when updating a task', async () => {
      const taskBefore = await db.queryOne(`SELECT * FROM tasks WHERE id = ?`, [taskId]);

      // Wait a bit to ensure timestamp difference
      await new Promise((resolve) => setTimeout(resolve, 100));

      await db.run(`UPDATE tasks SET completed = ? WHERE id = ?`, [1, taskId]);

      const taskAfter = await db.queryOne(`SELECT * FROM tasks WHERE id = ?`, [taskId]);
  ...
  describe('Tag CRUD Operations', () => {
    ...
    it('should set created_at and updated_at when creating a tag', async () => {
      const result = await db.run(`INSERT INTO tags (name, color) VALUES (?, ?)`, [
        'testtag',
        '#FF0000',
      ]);
      tagId = result.insertId;

      const tag = await db.queryOne(`SELECT * FROM tags WHERE id = ?`, [tagId]);
    ...
    it('should update updated_at when updating a tag', async () => {
      const tagBefore = await db.queryOne(`SELECT * FROM tags WHERE id = ?`, [tagId]);

      // Wait a bit to ensure timestamp difference
      await new Promise((resolve) => setTimeout(resolve, 100));

      await db.run(`UPDATE tags SET color = ? WHERE id = ?`, ['#00FF00', tagId]);

      const tagAfter = await db.queryOne(`SELECT * FROM tags WHERE id = ?`, [tagId]);
  ...
  describe('ShareNote CRUD Operations', () => {
    ...
    it('should set created_at and updated_at when creating a share_note', async () => {
      ...
      const share = await db.queryOne(`SELECT * FROM share_notes WHERE id = ?`, [shareId]);
    ...
    it('should update updated_at when updating a share_note', async () => {
      const shareBefore = await db.queryOne(`SELECT * FROM share_notes WHERE id = ?`, [shareId]);

      // Wait a bit to ensure timestamp difference
      await new Promise((resolve) => setTimeout(resolve, 100));

      await db.run(`UPDATE share_notes SET can_edit = ? WHERE id = ?`, [0, shareId]);

      const shareAfter = await db.queryOne(`SELECT * FROM share_notes WHERE id = ?`, [shareId]);
  ...
  describe('Invitation CRUD Operations', () => {
    ...
    it('should update updated_at when updating an invitation', async () => {
      const invitationBefore = await db.queryOne(`SELECT * FROM invitations WHERE id = ?`, [
        invitationId,
      ]);
      ...
      await db.run(`UPDATE invitations SET used = ? WHERE id = ?`, [1, invitationId]);

      const invitationAfter = await db.queryOne(`SELECT * FROM invitations WHERE id = ?`, [
        invitationId,
      ]);
  ...
  describe('PasswordResetToken CRUD Operations', () => {
    ...
    it('should set created_at and updated_at when creating a password_reset_token', async () => {
      ...
      const token = await db.queryOne(`SELECT * FROM password_reset_tokens WHERE id = ?`, [
        tokenId,
      ]);
    ...
    it('should update updated_at when updating a password_reset_token', async () => {
      const tokenBefore = await db.queryOne(`SELECT * FROM password_reset_tokens WHERE id = ?`, [
        tokenId,
      ]);
      ...
      await db.run(`UPDATE password_reset_tokens SET used = ? WHERE id = ?`, [1, tokenId]);

      const tokenAfter = await db.queryOne(`SELECT * FROM password_reset_tokens WHERE id = ?`, [
        tokenId,
      ]);
