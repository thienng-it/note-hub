...
class NoteService {
  ...
  static async getNotesForUser(
    ...
    if (searchQuery && searchQuery.length >= SEARCH_MIN_LENGTH && elasticsearch.isEnabled()) {
      const esResults: ElasticsearchResult = await elasticsearch.searchNotes(userId, searchQuery, {
      ...
      if (esResults?.notes) {
        ...
        const notes: NoteWithTags[] = await db.query(sql, validNoteIds);
    ...
    const notes: NoteWithTags[] = await db.query(sql, params);
   ...
   */
  static async getTagsForUser(userId: number): Promise<(Tag & { note_count: number })[]> {
    ...
    // Query database
    const tags: (Tag & { note_count: number })[] = await db.query(
      `
   ...
   */
  static async checkNoteAccess(noteId: number, userId: number): Promise<NoteAccess> {
    const note: NoteWithTags | null = await db.queryOne(
      `
      SELECT n.*,
        GROUP_CONCAT(t.name) as tag_names,
        GROUP_CONCAT(t.id) as tag_ids
    ...
    const share: ShareNote | null = await db.queryOne(
      `
   ...
   * Update note tags.
   */
  static async updateNoteTags(noteId: number, tagsString: string): Promise<void> {
    // Clear existing tags
    ...
    for (const tagName of tagNames) {
      // Get or create tag
      let tag: Tag | null = await db.queryOne(`SELECT * FROM tags WHERE name = ?`, [tagName]);
   ...
   * Get a note by ID.
   */
  static async getNoteById(noteId: number): Promise<NoteWithTags | null> {
    const note: NoteWithTags | null = await db.queryOne(
      `
      SELECT n.*,
        GROUP_CONCAT(t.name) as tag_names,
   ...
   */
  static async deleteNote(noteId: number, userId: number | null = null): Promise<void> {
   ...
   * Unshare a note.
   */
  static async unshareNote(noteId: number, shareId: number): Promise<void> {
   ...
   */
  static getExcerpt(body: string, length: number = 150): string {
    if (!body) return '';
   ...
   */
  static renderMarkdown(body: string): string {
    if (!body) return '';
