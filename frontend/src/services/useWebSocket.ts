...
import { websocketClient, ConnectionStatus, NoteUpdate, NoteDeleted } from '../services/websocketClient';
...
export function useWebSocket({ noteId, onNoteUpdate, onNoteDeleted, onUserJoined, onUserLeft }: UseWebSocketOptions = {}) {
  ...
  // Send note update
  const sendUpdate = useCallback((changes: any, version?: number) => {
    if (noteId) {
